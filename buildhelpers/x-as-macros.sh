...
#      macros changed to AS format and nothing else changed.
###############################################################################
...
while read line; do
    ...
    if [ -n "$(echo $line | egrep '^[^ ]* *MACRO')" ]; then
        ...
        modified_line=".macro $mname $mparams"
    else
        if [ -n "$(echo $line | grep 'ENDM')" ]; then
            ...
            if [ $inmacro -ne 0 ]; then
                # In the middle of a macro, so do appropriate parameter modifications
                idx=0
                for mparam in $mparams; do
                    idx=$(( $idx + 1 ))
                    modified_line=$(echo $modified_line | sed "s/$mparam/\\\\$mparam/g")
                done
            fi
        fi
    fi
